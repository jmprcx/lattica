...
> **Implementation update (post-assessment).** The codebase has since been **ported from Rust to Zig 0.16**, with every post-quantum and symmetric primitive (ML-KEM-768, ML-DSA-44, SHA3-256, ChaCha20-Poly1305) now sourced from `std.crypto`, and wallet keys derived **deterministically from the seed**. One thing changed materially for the worse in the port: the FRI-STARK proof — which had no `std.crypto` equivalent — is now a **clearly-labeled stub** rather than a real proof, deferred to a follow-up phase. The security analysis below already treated the proof system as the central gap, so its conclusions stand; just note that "the FRI proof is real and verifying" no longer describes the current Zig tree. Commands referencing `cargo` should be read as `zig build`.
