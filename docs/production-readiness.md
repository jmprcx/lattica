...
> **Implementation update (post-assessment).** The codebase has since been **ported from Rust to Zig 0.16**, with every post-quantum and symmetric primitive (ML-KEM-768, ML-DSA-44, SHA3-256, ChaCha20-Poly1305) now sourced from `std.crypto`, and wallet keys derived **deterministically from the seed**. The FRI-STARK proof — which has no `std.crypto` equivalent — was **reimplemented from scratch in Zig** (`src/stark.zig`): a genuine transparent, hash-based STARK over the Goldilocks field (NTT-interpolated trace LDE, Fiat-Shamir constraint composition, FRI low-degree testing, query openings binding composition to trace; no trusted setup). It is real and verifying, and now **zero-knowledge** (honest-verifier): the trace is blinded with `Z_H(x)·b(x)` so trace openings are uniform, and FRI runs on `CP + ζ·g` for a committed random `g` so the FRI openings leak nothing — proofs are randomized. It is unoptimized: a 1-in/1-out transfer carries a ~265 KB proof (prove ~170 ms, verify ~5 ms). Two of the gaps the assessment below flags as "production blockers" are now narrowed — the proof is real (gap rank 2 in §7) and zero-knowledge (gap 2 in §4.2) — though the ZK is PoC-grade and not formally proven. The remaining proof-system gaps still stand: it uses the algebraic `x → x³ + C` relation rather than a vetted one-way hash, proves only constraint (3) (membership/nullifier/balance remain node-enforced), and uses PoC-grade parameters. Commands referencing `cargo` should be read as `zig build`.
