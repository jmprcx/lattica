...
fn fuzz_htlc_verify_is_fail_closed_and_non_malleable() {
    ...
    assert_eq!(accepted, 0, "{accepted} mutated proofs verified — malleability/forgery risk");
    ...
    for _ in 0..iters {
        ...
        assert_ne!(verify(&proof, &b), 0, "verify accepted tampered public inputs at byte {idx}");
    }
    ...
    for len in [0usize, 1, 16, 31, 64, 1000, 100_000, proof.len() - 1, proof.len() + 1] {
        ...
        assert_ne!(verify(&g, &pib), 0, "garbage proof of len {len} must be rejected");
    }
    ...
    for delta in [-1i64, 1, -8, 8, 100] {
        ...
        assert_ne!(verify(&proof, &b), 0, "wrong-length public inputs ({n}) must be rejected");
    }
}
